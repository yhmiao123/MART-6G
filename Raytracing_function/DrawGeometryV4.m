function  DrawGeometryV4(Outputpath,OutputpathSca,simulat_params)

%V4 这个版本是面向室内场景，顶面给更透明了，同时先画无RIS；再画有RIS，同时优化了些画RIS的bug；
if nargin==0 %如果没有输入，即进行初始化
    clear;clc;
    Outputpath='E:\matlab_code\Raytracing_2024_V6_Thz_UI\Output\indoorRIS\Tx_1\Los1Ref2Dif1Pe1';
...
for index_Rx = 1:RxNum%每个接收点分别运行
    ...
    if simulat_params.isExistRIS==1
        % 进行RIS画图;只放有一个RIS板
        FaceNum_ris=size(risInfo.Face,2);
...
%             legend_labels{end+1} = 'RIS'; % 添加对应的图例标签
            ...
            set (gca,'position',[0.1,0.12,0.8,0.8] );
            set(gca,'FontSize',12); % 设置文字大小，同时影响坐标轴标注、图例、标题等。
            savefig([Outputpath '\RIS' num2str(iris) '.fig'])
