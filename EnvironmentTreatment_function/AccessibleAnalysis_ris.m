function [fp_avis,fD_avis]= AccessibleAnalysis_ris(Inputpath,TxPoint, params)

if nargin==0 %如果没有输入，即进行初始化
    clc;clear;
    Inputpath='E:\matlab_code\Raytracing_2024_V5_ris\Input\NorthtaipingzhuangCommunity\Rx_4';
    TxPoint=[107,-61,50];
end

%导入环境数据
load([Inputpath '\EnvirormentData.mat'],'FaceCoordinates','FaceEquation','FaceNum');
load([Inputpath '\risData.mat'],'risInfo');
%-----------可见面分析-----------
...
%-------进行RIS的可见面分析---------
for iris=1:size(risInfo.Face,2)
    RisPoint=risInfo.Face(1,iris).CenterPoint;
    visable_Faceindex2 = test_2D_1(RisPoint, FaceNum, FaceCoordinates,FaceEquation);%可见面的判断
    Visable_Faceindex1 = [Visable_Faceindex1;visable_Faceindex2];%将平行于地面的面与可见面的结果合并
    Visable_Faceindex1 = unique(Visable_Faceindex1,'row');%删去重复的
end
...
%--------进行绕射可见面索引----------
%1.多可见面Visable_Faceindex1进行面面组合索引；2.每个面面组合先判断能不能形成棱；
...
%-----二阶绕射面面组合索引-----
%对一阶绕射fD_avis(1)的面组合进行二次的循环索引（和二阶反射很像）；
