function visable_Faceindex = test_2D_2(Tx,Rx, FaceNum, FaceCoordinates, FaceEquation)
%传入起始点，面的个数，每个面的坐标
%%调试用的
%V1版本更正了Tx在屋顶时可见面失效的情况
%先找出在二维坐标系下Tx的可见范围
%V2版本解决室外场景Tx在边界上导致Tx可见区域过小的情况
%V2版本在二维情况下找出TX和RX的可见面
point = [];
All_segment=[];
...
for i = 1:FaceNum
    ...
    if any(judge(1,:) == 1)%只要有一个点满足条件，就将该面是为可见的
        visable_Faceindex1 = [visable_Faceindex1; i];
    end
end
...
index1 = unique(index1,'row');
visable_Faceindex1 = [visable_Faceindex1;index1];

%寻找RX可见面
%判断起点是否在建筑物上
RxOnBuilding = 0;
testpoint = [Rx(1,1:2), 0];
for indexFace = 1:FaceNum%逐面判断是否有穿透点
    if(roundn((FaceEquation(indexFace,1)*Rx(1,1)+FaceEquation(indexFace,2)*Rx(1,2)+FaceEquation(indexFace,3)*Rx(1,3)+FaceEquation(indexFace,4))*(FaceEquation(indexFace,1)*testpoint(1,1)+FaceEquation(indexFace,2)*testpoint(1,2)+FaceEquation(indexFace,3)*testpoint(1,3)+FaceEquation(indexFace,4)),-2) <0)
        pointPene = CrosspointLineFace(Rx(1,:),testpoint(1,:),FaceEquation(indexFace,:),FaceCoordinates(indexFace).Face(1,:));
    %Judge Crosspoint in Face 如果交点在该面上
        if (CpInFace2(pointPene,FaceEquation(indexFace,:),FaceCoordinates(indexFace).Face(:,:)))
            PeneFaceIndex = indexFace;
            RxOnBuilding = 1;
...
for i = 1:FaceNum
    ...
    %if all(judge(1,:) == 1)
    if any(judge(1,:) == 1)%只要有一个点满足条件，就将该面是为可见的
        visable_Faceindex2 = [visable_Faceindex2; i];
    end
end
...
index2 = unique(index2,'row');
visable_Faceindex2 = [visable_Faceindex2;index2];
visable_Faceindex = [visable_Faceindex1;visable_Faceindex2];
